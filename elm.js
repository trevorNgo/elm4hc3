...
var $author$project$Main$defaultEntries = _List_fromArray(
	[
		A4(
...
var $author$project$Main$nextEntry = function (model) {
	return _Utils_update(
		model,
		{current: model.current + 1});
};
var $author$project$Main$previousEntry = function (model) {
	return _Utils_update(
		model,
		{current: model.current - 1});
};
var $author$project$Main$selectAnswer = F3(
		...
		switch (msg.$) {
			...
			case 'Reset':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{current: 0, entries: $author$project$Main$defaultEntries, uid: $author$project$Main$defaultID}),
					$elm$core$Platform$Cmd$none);
			case 'NextEntry':
				return (_Utils_cmp(
					model.current,
					$elm$core$List$length(model.entries)) < 0) ? _Utils_Tuple2(
					$author$project$Main$nextEntry(model),
					$elm$core$Platform$Cmd$none) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 'PreviousEntry':
				return (model.current > 0) ? _Utils_Tuple2(
					$author$project$Main$previousEntry(model),
					$elm$core$Platform$Cmd$none) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			default:
				var selectedId = msg.a;
				var id = msg.b;
				return _Utils_Tuple2(
					$author$project$Main$nextEntry(
						A3($author$project$Main$selectAnswer, selectedId, id, model)),
					$elm$core$Platform$Cmd$none);
		}
	});
